		          ...
		          <table class="table mt-4">
		            ...
		            <tbody>
		            	@foreach($rejects as $reject)
							<tr class="text-center">
								<td><a href="{{ route('trace', ['tracer' => $reject->order->tracer]) }}">{{ $reject->order->shipment_category->name }}</a></td>
								<td>{{ $reject->order->quantity }}</td>
								<td>{{ $reject->package->name }}</td>
								<td>{{ $reject->order->destination->location }}</td>
								<td><a href="{{ route('duties.show', ['shipment.id' => $reject->id]) }}">view</a></td>
							</tr>
						@endforeach
		            </tbody>
		          </table>
